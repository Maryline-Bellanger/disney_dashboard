...
export default async function StarwarsFilms({params}: {params: {id: string}}) {
    const id = params.id;
    const [starwarsFilm] = await Promise.all([
        fetchFilmStarwarsById(id)
    ]);
  ...
  return (
    <main>
      <Breadcrumbs
        breadcrumbs={[
          { label: 'Films Disney', href: '/dashboard/starwars/films' },
          {
            label: 'Modifier',
