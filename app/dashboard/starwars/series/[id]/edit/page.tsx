...
export default async function StarwarsSeries({params}: {params: {id: string}}) {
    const id = params.id;
    const [starwarsSerie] = await Promise.all([
        fetchSerieStarwarsById(id)
    ]);
  ...
  return (
    <main>
      <Breadcrumbs
        breadcrumbs={[
          { label: 'Séries Disney', href: '/dashboard/starwars/series' },
          {
            label: 'Modifier',
